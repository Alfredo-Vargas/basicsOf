...
function Animal:new (height, weight, name, sound)
  -- To be able to add methods on top of this table ("class")
  setmetatable({}, Animal)
...
function Animal:toString()
  ...
  local animalStr = string.format("%s weighs %.1f lbs, is %.1f in tall and says %s", self.name, self.weight, self.height, self.sound)
...
function Cat:toString()
  ...
  local catStr = string.format("%s weighs %.1f lbs, is %.1f in tall, says %s and loves %s", self.name, self.weight, self.height, self.sound, self.favFood)
